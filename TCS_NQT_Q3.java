...
public class TcsQ3 {
	...
	public static void main(String[] args) {
		...
		p=sc.nextInt();
		System.out.println("Enter the total tenure");
		ty=sc.nextInt();
		double[] bank=new double[2];
		//for both the bank A and bank B.
		for(int i =0;i<2;i++)
		{
			...
			for(int k=0;k<n;k++)
			{
				...
				System.out.println("Enter the time period for "+interest+" interest"); //the total time period of all slabs = the total tenure.
				int t=sc.nextInt();
			    double ss=Math.pow((1+interest),(t*12));
			    sum=sum+((p*interest)/(1-(1/ss)));
			}
			bank[j++]=sum;
		}
		if(bank[0]<bank[1])
